package com.zzw.gmall.service.impl;

import com.alibaba.dubbo.config.annotation.Reference;
import com.netflix.hystrix.contrib.javanica.annotation.HystrixCommand;
import com.zzw.gmall.bean.UserAddress;
import com.zzw.gmall.service.OrderService;
import com.zzw.gmall.service.UserService;
import org.springframework.stereotype.Service;

import java.util.Arrays;
import java.util.List;

/**
 * 1、将服务提供者注册到注册中心（暴露服务）
 * 1）、导入 dubbo 依赖（2.6.2）\ 操作 zookeeper 的客户端 (curator)
 * 2）、配置服务提供者
 * <p>
 * 2、让服务消费者去注册中心订阅服务提供者的服务地址
 */
// 若要暴露服务，则使用 dubbo 的 @Service 注解
// 否则使用 spring 框架的 @Service 注解
@Service
public class OrderServiceImpl implements OrderService {

    // @Autowired
    // 远程引用
    // @Reference(url = "127.0.0.1:20881") // dubbo 直连
    @Reference(loadbalance = "random", timeout = 1000)
    private UserService userService;

    @HystrixCommand(fallbackMethod = "catchError")
    @Override
    public List<UserAddress> initOrder(String userId) {
        System.out.println("用户id：" + userId);
        // 1、查询用户的收货地址
        List<UserAddress> addressList = userService.getUserAddressList(userId);
        if (addressList != null) {
            for (UserAddress userAddress : addressList) {
                System.out.println(userAddress.getUserAddress());
            }
        }
        return addressList;
    }

    public List<UserAddress> catchError(String userId) {
        return Arrays.asList(new UserAddress(10, "1", "测试地址", "测试", "测试", "Y"));
    }
}
